...
# Function to find country from latitude and longitude
find_country <- function(lat, long) {
  world <- ne_countries(scale = "medium", returnclass = "sf")
  point <- st_point(c(long, lat))
  point <- st_sfc(point, crs = st_crs(world))
  country <- world[st_intersects(world, point), "name_long"]
  return(as.character(country))
}

stp.dat <- stp.dat %>% 
  dplyr::mutate(country = map2_chr(LATITUDE,LONGITUDE, find_country))
